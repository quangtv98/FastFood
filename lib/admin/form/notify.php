<div class="container text-center p-0 mt-4">
    ...
    <?php 
        if(isset($_SESSION['id_staff'])){
            ...
            $stmt=$conn->prepare('SELECT id_notify, status FROM notify_staff WHERE id_staff=:id_staff ORDER BY id_notify DESC');
        ...
        <table class="col-md-12 table-hover shadow">
            ...
            foreach($result as $row_s){
                ...
                <tr>
                    ...
                    <td><a href="lib/admin/process/mark_notify.php?id_notify=<?php echo $row['id_notify'] ?>" class="text-info">Đánh dấu là đã đọc</a></td>
                    <?php }else{ ?>
                    <td><p>Đã Xem</p></td>
                    <?php } ?>
                    <td><a href="lib/admin/process/del_notify.php?id_notify=<?php echo $row['id_notify'] ?>" class="text-danger"><i class="fa fa-trash-o px-1" aria-hidden="true"></i></a></td>
                </tr>
            <?php }} ?>
            </tbody>
        </table>
